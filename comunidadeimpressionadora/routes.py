...
def login_cadastro():
    ...
    if form_login.validate_on_submit() and 'botao_submit_login' in request.form:
        ...
        if usuario and bcrypt.check_password_hash(usuario.senha, form_login.senha.data):
            ...
            flash(f'Falaha no login. Email ou senha incorretos.', 'alert-danger')
    if form_criarconta.validate_on_submit() and 'botao_submit_criarconta' in request.form:
        ...
        return redirect(url_for('home'))
    return render_template('login_cadastro.html', form_login=form_login, form_criarconta=form_criarconta)
...
def excluir_post(post_id):
    ...
    if current_user == post.autor:
        ...
        database.session.commit()
        flash('Post apagado com sucesso', 'alert-success')
        return redirect(url_for('home'))
    else:
        flash('Você não tem autorização para apager esse post.', 'alert-danger')
